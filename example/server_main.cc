...
int main(int argc, char *argv[])
{
	...
	EchoServiceServer server(&params);
	server.register_echo(
	...
	{
		printf("rpc server started on %u (service=%s method=%s)\n",
			   port,
			   EchoServiceServer::service_name().c_str(),
			   EchoServiceServer::echo_method_name().c_str());
		wait_group.wait();
		server.stop();
		return 0;
	}
